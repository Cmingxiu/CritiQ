...
#         # 保留第一个和第二个引号
#         char_list[quote_indices[0]] = '"'  # 第一个引号
#         char_list[quote_indices[1]] = '"'  # 第二个引号

#         # 替换第3到倒数第3个引号为单引号
#         for idx in quote_indices[2:-2]:
#             char_list[idx] = "'"

#         # 保留倒数第一个和倒数第二个引号
#         char_list[quote_indices[-2]] = '"'  # 倒数第二个引号
#         char_list[quote_indices[-1]] = '"'  # 倒数第一个引号

#         # 将字符列表合并为字符串
#         text = ''.join(char_list)

#     # 确保 JSON 字符串以大括号开头并结束
...
#         return result
#     except Exception as e:
#         raise ValueError(f"Failed to parse JSON: {cleaned_text}") from e
