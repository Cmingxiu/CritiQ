...
```""",
    ...
    MID_REFLECTION_PROMPT="""请分析这个错误案例以及 worker agent 的分析。基于你的分析，请提供如何改进该指标描述的批判性意见，以指导 worker agent 避免错误或精确识别指标不适用的情况，请注意，修改意见应该从该指标本身出发，不应该考虑和其他指标之间得关系。
...
```
“Y” 表示高质量，“N” 表示低质量，如果你不确定，请在 answer 里回复 **UNSURE**.""",
...
# ```
# “Y” 表示高质量，“N” 表示低质量，如果你不确定，请在 answer 里回复 **UNSURE**.""",
#     ZERO_ONE_WORKER_PROMPT="基于指标“**{criterion}**”，这段数据是高质量数据吗（请注意，英文也属于普通话和方言。）？\n\n{criterion}：{description}\n\n[DATA]\n{text}\n[/DATA]",
#     CRITERION_NAME_DESC_FORMAT_TEMPLAT="\n\n[CRITERION]\n指标：{name}\n\n描述：{desc}\n[/CRITERION】",
# )

WORKFLOW_LANGUAGE = Language(os.getenv("WORKFLOW_LANG", "zh"))
